 ...
 <div id="admin-update-book" class="admin-panel flex-1">
                        <div class="p-6">
                            <div class="max-w-4xl mx-auto">
                                <div class="mb-8">
                                    <h2 class="text-2xl font-bold text-gray-800">Update Book Information</h2>
                                    <p class="text-gray-600">Edit book details and information</p>
                                </div>

                                <div class="bg-white rounded-xl shadow-lg p-8">

                                    <form action="/admin/updateBook" method="POST" class="space-y-6">
                                        <input type="hidden" name="id" value="<?= $book["id"] ?>">
                                        <div class="grid md:grid-cols-2 gap-6">
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700 mb-2">Book Title</label>
                                                <input type="text" name="title" value="<?= $book["title"] ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700 mb-2">Author</label>
                                                <input type="text" name="author" value="<?= $book["author"] ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                            </div>
                                            <div>
                                                <label class="block text-sm font-medium text-gray-700 mb-2">Publication Year</label>
                                                <input type="number" name="year" value="<?= $book["year"] ?>" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                                            </div>
                                        </div>
